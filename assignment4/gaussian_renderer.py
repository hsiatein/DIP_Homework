...
class GaussianRenderer(nn.Module):
    ...
    def compute_gaussian_values(
        ...
        exponent = -torch.einsum('nhwi,nij,nhwj->nhw', dx, inv_covs2D, dx)/2  # (N, H, W)
        coef = 1 / (2* np.pi * torch.sqrt(torch.det(covs2D)))  # (N,)
        gaussian = coef.view(N, 1, 1) * torch.exp(exponent)  # (N, H, W)
        return gaussian
    ...
    def forward(
        ...
        ### FILL:
        ### weights = ... # (N, H, W)
        alphas_cp = torch.cumprod(1 - alphas, dim=0)  # (N, H, W)
        alphas_cp = torch.cat([torch.ones(1, self.H, self.W, device=alphas.device), alphas_cp[:-1]], dim=0)  # Shift by 1
        weights = alphas * alphas_cp  # (N, H, W)
        # 8. Final rendering
