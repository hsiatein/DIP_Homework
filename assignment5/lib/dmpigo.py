...
from torch_scatter import scatter_add, segment_coo, gather_coo

from . import grid
from .dvgo import Raw2Alpha, Alphas2Weights, render_utils_cuda, color_aware_voxel_smooth_cuda
...
class DirectMPIGO(torch.nn.Module):
    ...
    def color_aware_voxel_smooth_add_grad(self,weight,dense_mode):
        wxy = weight * self.world_size[:2].max() / 128
        wz = weight * self.mpi_depth / 128
        color_aware_voxel_smooth_cuda.color_aware_voxel_smooth_add_grad(self.k0.grid.detach(),self.density.grid,self.density.grid.grad,wxy,wxy,wz,dense_mode,True)
    ...
    def sample_ray(self, rays_o, rays_d, near, far, stepsize, **render_kwargs):
        ...
        ray_pts, mask_outbbox = render_utils_cuda.sample_ndc_pts_on_rays(
            rays_o, rays_d, self.xyz_min, self.xyz_max, near, far, N_samples)
        mask_inbbox = ~mask_outbbox # n_rays *  n_pt per ray
    ...
    def forward(self, rays_o, rays_d, viewdirs, global_step=None,scale=0, **render_kwargs):
        '''Volume rendering
        ...
        density = self.density(ray_pts,scale) + self.act_shift(ray_pts)
        ...
        steps = step_id / N_samples
        s = (step_id+0.5) / N_samples
        
        if render_kwargs.get('render_depth', False) or render_kwargs.get('render_normal', False) or render_kwargs.get('render_normal_w_grad', False):
            depth = segment_coo(
                    src=(weights * step_id),
                    index=ray_id,
                    out=torch.zeros([N]),
                    reduce='sum')
            # depth += (alphainv_last * N_samples)
            ret_dict.update({'depth': depth})
        
        if render_kwargs.get('render_normal', False) and not render_kwargs.get('render_normal_w_grad', False):
            rays_surf=(rays_o+rays_d*(depth.unsqueeze(dim=-1))/N_samples).detach_().requires_grad_()
            surf_density = self.density(rays_surf)
            rays_normal = AG.grad(surf_density.sum(),rays_surf)[0]
            rays_normal=-1*F.normalize(rays_normal)
            ret_dict.update({'normal':rays_normal})  

        if render_kwargs.get('render_normal_w_grad', False) and not render_kwargs.get('render_normal', False):
            rays_surf=(rays_o+rays_d*(depth.unsqueeze(dim=-1))/N_samples).requires_grad_()
            surf_density = self.density.grid_sample(rays_surf)
            rays_normal = AG.grad(surf_density.sum(),rays_surf)[0]
            rays_normal=-1*F.normalize(rays_normal)
            ret_dict.update({'normal':rays_normal})  
                 
        if render_kwargs.get('point2ray', False):
            ones = torch.ones(ray_id.size(), dtype=torch.uint8)
            count = scatter_add(ones, ray_id, dim=0)
            weights_ray = torch.split(weights,count.tolist())
            alpha_ray = torch.split(alpha,count.tolist())
            rgb_ray = torch.split(rgb,count.tolist())
            ret_dict.update({
                'weights_ray':weights_ray,
                'alpha_ray':alpha_ray,
                'rgb_ray':rgb_ray
            }
            )
        ...
        if render_kwargs.get('render_depth_real', False):
            with torch.no_grad():
                depth = segment_coo(
                        src=(weights * step_id),
                        index=ray_id,
    ...
    def voxel_count_views(self, rays_o_tr, rays_d_tr, imsz, near, far, stepsize, downrate=1, irregular_shape=False):
        print('dmpigo: voxel_count_views start')
        eps_time = time.time()     
        count = torch.zeros_like(self.density.get_dense_grid()).long()
        device = count.device
        for rays_o_, rays_d_ in zip(rays_o_tr.flatten(1,2).split(imsz), rays_d_tr.flatten(1,2).split(imsz)):
            ones = grid.DenseGrid(1, self.world_size, self.xyz_min, self.xyz_max)
            for rays_o, rays_d in zip(rays_o_.squeeze().split(8192), rays_d_.squeeze().split(8192)):
                ray_pts, ray_id, step_id, N_samples = self.sample_ray(
                        rays_o=rays_o.to(device), rays_d=rays_d.to(device), near=near, far=far, stepsize=stepsize)
                ones(ray_pts).sum().backward()
            count.data += (ones.grid.grad > 1)
        eps_time = time.time() - eps_time
        print('dmpigo: voxel_count_views finish (eps time:', eps_time, 'sec)')
        return count
    ...
    def set_inc_mask(self, lower, upper): 
        ...
        mask=(xyz[0]>=lower[0]) & (xyz[0]<=upper[0]) & (xyz[1]>=lower[1]) & (xyz[1]<=upper[1]) & (xyz[2]>=lower[2]) & (xyz[2]<=upper[2])
