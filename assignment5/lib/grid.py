...
class DenseGrid(nn.Module):
    ...
    def forward(self, xyz,scale=0):
        ...
        #添加池化层下采样
        if scale>0:
            pool = nn.MaxPool3d(kernel_size=(2, 2, 2))
            grid_pooled=pool(self.grid)
            for _ in range(scale-1):
                grid_pooled=pool(grid_pooled)
            out = F.grid_sample(grid_pooled, ind_norm, mode='bilinear', align_corners=True) # 1*C*1*1*N
        else:
            out = F.grid_sample(self.grid, ind_norm, mode='bilinear', align_corners=True) # 1*C*1*1*N
        out = out.reshape(self.channels,-1).T.reshape(*shape,self.channels) # N*C
        if self.channels == 1:
            out = out.squeeze(-1)
        return out
    ...
    def get_index(self,xyz):
        '''
        xyz: global coordinates to query
        '''
        xyz = xyz.reshape(1,1,1,-1,3)
        xyz_index=([*(self.world_size-1)]*((xyz - self.xyz_min) / (self.xyz_max - self.xyz_min))).round().long()
        return xyz_index      
    ...
    mode: str = "trilinear",
        ...
        if(mode=='nearest'):
            xyz_index=((self.world_size-1)*((xyz - self.xyz_min) / (self.xyz_max - self.xyz_min))).round().long()
            out=self.grid[0,:,xyz_index[...,0],xyz_index[...,1],xyz_index[...,2]] # C*N? 
            out = out.T # N*C
        elif(mode=="trilinear"):
            xyz=((self.world_size-1)*((xyz - self.xyz_min) / (self.xyz_max - self.xyz_min)))
            x0y0z0=xyz.floor().squeeze()
            x0=x0y0z0[...,0].long()
            y0=x0y0z0[...,1].long()
            z0=x0y0z0[...,2].long()
            x1y1z1=xyz.ceil().squeeze()
            x1=x1y1z1[...,0].long()
            y1=x1y1z1[...,1].long()
            z1=x1y1z1[...,2].long()
            ...
            c00=self.grid[0,:,x0,y0,z0]*(1-dx)+self.grid[0,:,x1,y0,z0]*dx
            c01=self.grid[0,:,x0,y0,z1]*(1-dx)+self.grid[0,:,x1,y0,z1]*dx
            c10=self.grid[0,:,x0,y1,z0]*(1-dx)+self.grid[0,:,x1,y1,z0]*dx
            c11=self.grid[0,:,x0,y1,z1]*(1-dx)+self.grid[0,:,x1,y1,z1]*dx
