...
def _compute_bbox_by_cam_frustrm_bounded(cfg, HW, Ks, poses, i_train, near, far):
    ...
    # test1 = HW[i_train]
    # test2 = Ks[i_train]
    # test3 = poses[torch.tensor(i_train, dtype=torch.long)]
    for (H, W), K, c2w in zip(HW[i_train], Ks[i_train], poses[torch.tensor(i_train, dtype=torch.long)]):
        rays_o, rays_d, viewdirs = dvgo.get_rays_of_a_view(
                H=H, W=W, K=K, c2w=c2w,
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, 
    ...
    if len(cfg.sparse_train.hardcode_train_views):
        ...
        if not args.no_log:
            writer.add_images('sparse_train/inputs',images[torch.tensor(i_train, dtype=torch.long, device='cpu')],dataformats='NHWC')
            # wandb.log({'sparse_train/inputs':i_train},commit=False)
    ...
    for global_step in trange(1+start, 1+cfg_train.N_iters):
        ...
        if cfg_train.N_rand_sample:
            if cfg_train.ray_sampler == 'random':
                ...
                idx_img_new = np.random.randint(0, len(i_train), size=(n_patches, 1))
                # idx_img_new = np.random.choice(i_train, size=(n_patches, 1))
                ...
                rays_o_rd_tr_test = rays_o_rd_tr[idx_img, patch_idx[Ellipsis, 1], patch_idx[Ellipsis, 0]]
        ...
        loss = 0
        for scale_num in range(3):
            render_result = model(
            rays_o, rays_d, viewdirs,
            global_step=global_step, is_train=True,scale=scale_num,
            **render_kwargs)
            ...
            if cfg_train.weight_reprojection > 0 and scale_num==0:
                rgb = images[i_train].to('cpu' if cfg.data.load2gpu_on_the_fly else device)
                # reprojection_loss = reprojection_error(rays_o_rd_patch, rays_d_rd_patch, viewdirs_rd_patch, render_rd_patch_result['raw_rgb'], render_rd_patch_result['depth'], poses[idx_img, :, :], poses[idx_img_new, :, :], Ks[0], rgb, idx_img_new)
                reprojection_loss = reprojection_error(rays_o_rd_patch, rays_d_rd_patch, viewdirs_rd_patch, render_rd_patch_result['rgb_marched'], render_rd_patch_result['depth'], poses[idx_img, :, :], poses[idx_img_new, :, :], Ks[0], rgb, idx_img_new)
                loss += cfg_train.weight_reprojection * reprojection_loss
            ...
            if cfg_train.weight_distortion > 0 and scale_num==0:
            ...
            if global_step<cfg_train.tv_depth_before and global_step>cfg_train.tv_depth_after and cfg_train.weight_tv_depth >0 and scale_num==0:
            ...
            if cfg_train.weight_inverse_depth >0 and scale_num==0:
            ...
            if cfg_train.weight_normal > 0 and scale_num==0:
...
def cfg_override(args:argparse.Namespace,cfg:mmengine.Config):
